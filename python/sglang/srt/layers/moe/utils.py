...
def initialize_moe_config():
    ...
    Reads the bags rather than the startup record: `moe_a2a_backend` and its
    siblings are resolution's answers, and the record carries them only while
    declarations materialize onto it. Called once per process after publish
