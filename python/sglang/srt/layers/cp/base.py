...
def init_cp_strategy(
    *, enable_prefill_cp: bool, cp_size: int, cp_strategy: str
    ...
    Takes the three values rather than a config object: resolution calls this
    from inside `__post_init__`, where the bags do not exist yet, and
    `get_cp_strategy` calls it lazily in a worker, where the record is not the
    place to read them. Each caller reads from the source it has.
    """
    global _STRATEGY
