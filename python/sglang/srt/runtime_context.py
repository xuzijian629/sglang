...
class ParallelContext:
    ...
    def config(self) -> _ConfigBag:
        ...
        Read through the slot rather than ``config_bag()``: ``parallel`` is
        deliberately outside the per-role namespace table (every process reads
        topology config), so the role check must not run here. The body stays
...
def _build_config_bags(server_args: Any) -> dict:
    ...
    the caller supplied -- rather than from the field, which carries the raw
    input. Returns
...
def assert_published(server_args, *, role: str) -> RuntimeContext:
    ...
    functions. The three constructors that call this (`ModelRunner`,
    `TokenizerManager`, `MMEncoder`) used to publish here instead, which put
    "are the bags available?" at the mercy of which constructor had run, and
    made a late `publish` re-project the bags over a live process -- silently
    discarding every `override()` taken since, and the provenance with it.

    So a constructor arriving here unpublished means an entry is missing.
    Publishing at this point would appear to work, which is exactly why this
    raises instead.
    """
