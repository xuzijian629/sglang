...
_KNOWN_ENTRIES = frozenset(
    {
        ...
        # Process entries that used to leave publishing to a constructor's
        # defensive `ensure_published`: the multi-tokenizer worker, the
        # benchmark work functions (run inline or spawned per rank), and the
        # encoder's gRPC / spawned-TP / spawned-DP entries.
