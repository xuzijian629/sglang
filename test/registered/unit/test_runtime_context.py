...
class TestAssertPublished(_IsolatedServerArgs):
    ...
    `ModelRunner`, `TokenizerManager` and `MMEncoder` used to publish
    defensively, because each could be built with nothing published first.
    Inside a process that had already published, that second publish
    re-projected the bags -- discarding every `override()` taken since, and the
    provenance log with it. The entries publish now, so these constructors
    assert instead, and an unpublished one fails loud rather than working by
    accident.
    """
    ...
    def test_an_empty_slot_fails(self):
        """The case the defensive publish used to cover silently."""
        reset_context()
        record = self._record(grammar_backend="xgrammar")
