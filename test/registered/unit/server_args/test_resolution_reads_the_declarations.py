...
def _declared_fields():
    ...
    A keyword on a `declare_*` call is only the first shape. The model-override
    and post-process passes never name their fields as keywords: they build a
    mapping (`MODEL_OVERRIDES` literals, `overrides["dtype"] = ...`,
    `return {"fp8_gemm_runner_backend": ...}`) that
    `collect_model_override_declarations` / `run_post_process_pass` append to the
    stash. Deriving from keywords alone leaves `dtype`,
    `fp8_gemm_runner_backend`, `speculative_moe_runner_backend` and sixteen more
    outside the subject set -- which is exactly how a member reading
    `self.speculative_moe_runner_backend` passed this file once already.
    """
    fields = set()
...
def _record_arguments(node, aliases=frozenset()):
    ...
    for arg in node.args:
        if isinstance(arg, ast.Name) and arg.id in ("self", "server_args", "sa"):
            out.append(arg.id)
        elif isinstance(arg, ast.Attribute) and arg.attr in ("server_args", "sa"):
...
def _record_aliases(function):
    ...
    `_sa = getattr(runner, "server_args", None)` and `cfg = get_server_args()`
    both put the record behind a name the argument scan does not recognise, so
    a later `getattr(_sa, "<decided leaf>")` reads what the operator typed.
    """
    aliases = set()
    for node in ast.walk(function):
        ...
        if not isinstance(target, ast.Name):
            continue
        value = node.value
        if isinstance(value, ast.Attribute) and value.attr in ("server_args", "sa"):
            aliases.add(target.id)
        elif isinstance(value, ast.Call) and isinstance(value.func, ast.Name):
            if value.func.id in _RECORD_ACCESSORS:
                aliases.add(target.id)
            elif (
                value.func.id == "getattr"
                and len(value.args) >= 2
                and isinstance(value.args[1], ast.Constant)
                and value.args[1].value in ("server_args", "sa")
            ):
                aliases.add(target.id)
    return aliases
    ...
    for scope, aliases in scopes:
        for node in ast.walk(scope):
            if isinstance(node, ast.Call):
                ...
                record(node.lineno, f"{node.value.id}.{node.attr}")
    return offenders
...
class TestResolutionReadsTheDeclarations(CustomTestCase):
    ...
    def test_no_reader_hands_the_record_to_a_config_helper(self):
        ...
        decided = _declared_fields()
        offenders = []
        roots = [_SRT] + [_SRT.parent / d for d in ("benchmark", "lang")]
        for root in roots:
            if not root.exists():
                continue
    ...
    def test_the_scan_sees_every_spelling_that_reached_production(self):
        """The negative pin, kept next to the scanner rather than in a comment.

        Round 3 converted two MiniMax reads and one world-size read; the claim
        that the guard catches them was true of the helper hand-off and false of
        the `getattr(alias, leaf)` half. Pinning the spellings here is what makes
