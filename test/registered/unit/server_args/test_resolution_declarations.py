...
stash says. The second check is what the stash is measured against:
a declared field and its stash entry can only disagree if something assigned
the field behind the stash's back. A third check runs the other way: every
field resolution moved has to be explained by the stash, which covers the
spellings a source scan cannot see.
"""
...
class TestResolutionDeclarations(CustomTestCase):
    ...
    def test_a_declaration_only_resolver_leaves_the_field_alone(self):
        ...
        answer and the field still holds what the caller passed. This used to
        compare the projection against the record leaf for leaf, which was the
        right check while the declarations were replayed onto the fields at the
        end of resolution; that replay is gone, so the comparison would now
    ...
    def test_the_stash_is_what_the_projection_answers(self):
        ...
        The stash is the resolution result now: the fields are no longer
        replayed at the end, so a disagreement here means the projection reads
        something other than the declarations -- which is what every bag is
        built from.
        """
        mismatches = []
        for shape in _SHAPES:
