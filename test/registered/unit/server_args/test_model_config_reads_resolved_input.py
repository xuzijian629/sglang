...
def _server_args_names(tree, path):
    ...
    declaration stash over the fields) rather than off the record: the
    declaration-only resolvers never write the field, so a field read there
    answers with the raw input. `cfg.dtype` after `cfg = resolving_view(sa)` is
