...
class TestFlashInferCommFusion(CustomTestCase):
    ...
    The public entry above it takes no arguments now -- it reads
    `exec.comm.flashinfer_allreduce_fusion_backend` and `parallel.nnodes` off the
    published bags -- so the cases here drive the dispatch directly rather than
    publishing a config per architecture.
    """
